...
function callTakeHostage()
	...
	if not canTakeHostage then 
		exports['mythic_notify']:DoHudText('inform', 'You need a pistol with ammo to take someone as hostage!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				exports['mythic_notify']:DoHudText('inform', 'No one nearby to take as hostage!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
			end
		else
			exports['mythic_notify']:DoHudText('inform', 'No one nearby to take as hostage!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
		end
