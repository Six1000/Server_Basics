    ...
    if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
        ...
        if ( IsPedSittingInAnyVehicle( ped ) ) then 
            ...
            if ( DoesEntityExist( vehicle ) ) then 
                ...
                exports['mythic_notify']:DoHudText('inform', 'You must be in or near a vehicle to delete it!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
            end 
        end 
    end 
end )
...
function DeleteGivenVehicle( veh, timeoutMax )
    ...
    if ( DoesEntityExist( veh ) ) then
        exports['mythic_notify']:DoHudText('inform', 'Failed to delete vehicle, trying again...', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
        ...
        while ( DoesEntityExist( veh ) and timeout < timeoutMax ) do 
            ...
            if ( not DoesEntityExist( veh ) ) then 
                exports['mythic_notify']:SendAlert('inform', 'Vehicle Deleted!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
            end 
            ...
            if ( DoesEntityExist( veh ) and ( timeout == timeoutMax - 1 ) ) then
                exports['mythic_notify']:DoHudText('inform', 'Failed to delete vehicle, please retry!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
            end 
        end 
    else 
        exports['mythic_notify']:DoHudText('inform', 'Vehicle Deleted!', 2500, { ['background-color'] = '#76b5c5', ['color'] = '#ffffff' })
    end 
end 
